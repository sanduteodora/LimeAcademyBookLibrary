...
contract BookLibrary is Ownable {
    ...
    function addBooks(string memory _name, uint _copies) public {
        if(!isPresent[_name]) {
    ...
    function borrowBook(string memory _name,  uint copies ) public {
        ...
        emit BorrowBook(_name,nameToId[_name], copies,"You have borrowed books");
    }


    function returnBook(string memory _name, uint _copies) public {
        Book storage book = books[nameToId[_name]];
    ...
    function returnBookArrayLength() public view returns (uint _bookArrayLength) {

        return listOfBookIds.length;

    }

    function returnTotalBookCount() public view returns(uint _currentBookCount) {
      return bookCount;
  }
